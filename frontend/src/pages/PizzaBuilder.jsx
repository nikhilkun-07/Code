...
import { Pizza, Check, ShoppingCart, ArrowLeft, ArrowRight, Loader, Plus } from 'lucide-react'
...
const PizzaBuilder = () => {
  ...
  const fetchIngredients = async () => {
    try {
      ...
      const categorized = {
        ...
        sauce: data.ingredients?.filter(ing => ing.category === 'sauce') || [],
        cheese: data.ingredients?.filter(ing => ing.category === 'cheese') || [],
        veggie: data.ingredients?.filter(ing => ing.category === 'veggie') || [],
  ...
  const calculatePrice = (selections) => {
    ...
    if (selections.base) {
      const baseIngredient = ingredients.base.find(b => b.name === selections.base)
      price += baseIngredient?.price || 0
    }
    
    if (selections.sauce) {
      const sauceIngredient = ingredients.sauce.find(s => s.name === selections.sauce)
      price += sauceIngredient?.price || 0
    }
    
    if (selections.cheese) {
      const cheeseIngredient = ingredients.cheese.find(c => c.name === selections.cheese)
      price += cheeseIngredient?.price || 0
    }
    
    selections.veggies.forEach(veggie => {
      const veggieIngredient = ingredients.veggie.find(v => v.name === veggie)
        ...
        >
          <div className="flex flex-col items-center space-y-2">
            ...
            <div className="flex items-center justify-between w-full">
              ...
              {selectedList.includes(item.name) && <Check className="h-4 w-4 text-blue-600" />}
            </div>
  ...
  const renderContent = () => {
    ...
    switch (currentStep) {
      case 1: 
        return (
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Choose Your Base</h3>
            {ingredients.base.length > 0 ? (
              renderIngredientButtons(ingredients.base, handleBaseSelect, pizza.base)
            ) : (
              <p className="text-gray-500 text-center py-8">No bases available</p>
            )}
          </div>
        )
      case 2: 
        return (
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Choose Your Sauce</h3>
            {ingredients.sauce.length > 0 ? (
              renderIngredientButtons(ingredients.sauce, handleSauceSelect, pizza.sauce)
            ) : (
              <p className="text-gray-500 text-center py-8">No sauces available</p>
            )}
          </div>
        )
      case 3: 
        return (
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Choose Your Cheese</h3>
            {ingredients.cheese.length > 0 ? (
              renderIngredientButtons(ingredients.cheese, handleCheeseSelect, pizza.cheese)
            ) : (
              <p className="text-gray-500 text-center py-8">No cheeses available</p>
            )}
          </div>
        )
      case 4:
        return (
          <div className="space-y-8">
            <div>
              <h3 className="text-xl font-semibold text-gray-800 mb-4">Add Vegetables</h3>
              {ingredients.veggie.length > 0 ? (
                renderToggleIngredientButtons(ingredients.veggie, pizza.veggies, handleVeggieToggle)
              ) : (
                <p className="text-gray-500 text-center py-4">No vegetables available</p>
              )}
            </div>
            <div>
              <h3 className="text-xl font-semibold text-gray-800 mb-4">Add Meat</h3>
              {ingredients.meat.length > 0 ? (
                renderToggleIngredientButtons(ingredients.meat, pizza.meat, handleMeatToggle)
              ) : (
                <p className="text-gray-500 text-center py-4">No meat available</p>
              )}
            </div>
            
            <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
              <h3 className="text-xl font-semibold text-gray-800 mb-4">Your Custom Pizza</h3>
              <div className="space-y-2 text-gray-700 mb-6">
                <p><strong>Base:</strong> {pizza.base}</p>
                <p><strong>Sauce:</strong> {pizza.sauce}</p>
                <p><strong>Cheese:</strong> {pizza.cheese}</p>
                {pizza.veggies.length > 0 && <p><strong>Veggies:</strong> {pizza.veggies.join(', ')}</p>}
                {pizza.meat.length > 0 && <p><strong>Meat:</strong> {pizza.meat.join(', ')}</p>}
              ...
              {addedToCart ? (
                <div className="text-center space-y-4">
                  <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg">
                    <p className="font-semibold">✓ Pizza added to cart!</p>
                  </div>
                ...
                <div className="space-y-4">
                  <button
                    ...
                    disabled={loading || !pizza.base || !pizza.sauce || !pizza.cheese}
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            {[1, 2, 3, 4].map((step) => (
              <React.Fragment key={step}>
                ...
                {step < 4 && <div className={`w-8 sm:w-12 h-1 ${currentStep > step ? 'bg-blue-600' : 'bg-gray-200'}`} />}
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-between mb-6 text-sm text-gray-600 px-4">
          {steps.map((label, i) => (
            <span key={i} className={`${currentStep >= i + 1 ? 'text-blue-600 font-semibold' : ''}`}>
              {label}
            </span>
          ))}
        </div>
